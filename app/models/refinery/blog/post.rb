...
module Refinery
  module Blog
    class Post < ActiveRecord::Base
      ...
      belongs_to :teaser_image, :class_name => '::Refinery::Image', :foreign_key => :teaser_image_id
      ...
      attr_accessible :teaser_image_id
    ...
    class Translation
      ...
      def next_in_category
        self.class.next_in_category(self)
      end

      def prev_in_category
        self.class.previous_in_category(self)
      end
      ...
      class << self
        ...
        def previous_in_category(item)
          Category.find(item.categories.first.id).posts.published_before(item.published_at).first
        end
        ...
        def next_in_category(current_record)
          Category.find(current_record.categories.first.id).posts.where(["published_at > ? and draft = ?", current_record.published_at, false]).reorder('published_at ASC').with_globalize.first
        end
